...
from rag.ingestion import Chunk, chunk_by_paragraphs, load_txt
...
def _resolve_profile_doc(doc: str | Path | None) -> Path:
    """Ruta absoluta al TXT de LinkedIn de la persona representada."""
    if doc is not None and str(doc).strip():
        candidate = Path(doc)
    else:
        env = os.getenv("REPRESENTED_LINKEDIN_DOC", "").strip()
        candidate = Path(env) if env else Path("data") / "linkedin_Ed_Donner.txt"
...
def _search_linkedin_text_fallback(query: str, max_snippets: int = 5) -> str | None:
    ...
    if not _profile_doc_path.is_file():
        return None
    text = _profile_doc_path.read_text(encoding="utf-8")
    q = query.strip().lower()
    if not q:
        return None
...
def _build_profile_chunks() -> list[Chunk]:
    ...
    if not _profile_doc_path.is_file():
        return []
    doc = load_txt(str(_profile_doc_path))
    doc.metadata = {
        **doc.metadata,
...
def _sync_tool_descriptions() -> None:
    ...
    TOOLS_REGISTRY["search_docs"]["description"] = (
        "Busca en el export de LinkedIn de la persona representada usando RAG híbrido "
        f"(BM25 + vector; {rel}). Argumento: consulta en lenguaje natural."
    )
    TOOLS_REGISTRY["lookup"]["description"] = (
        "Busca un término o frase corta en el mismo documento de perfil que usa search_docs "
